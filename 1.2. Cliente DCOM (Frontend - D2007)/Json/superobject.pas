...
function TryISO8601ToDate(const ISOStr: string; out Date: TDateTime): Boolean;
var
  Year, Month, Day: Word;
begin
  Result := False;
...
function JavaToDelphiDateTime(const JavaMillis: Int64): TDateTime;
const
  MSecsPerDay = 24 * 60 * 60 * 1000; // milisegundos por día
  DelphiEpoch: TDateTime = 25569;     // 30/12/1899 en TDateTime = 0
  UnixEpoch: TDateTime = 25569;       // 30/12/1899 + ajuste, ver nota
begin
  // Convertir milisegundos a días
  // Delphi TDateTime cuenta días desde 30/12/1899, Unix/Java desde 1/1/1970
  Result := (JavaMillis / MSecsPerDay) + EncodeDate(1970,1,1);
end;
...
begin
  ...
  case obj.DataType of
    stInt:
      begin
        // Suponemos que es un entero tipo timestamp Java
        TmpInt := obj.AsInteger;
        dt := JavaToDelphiDateTime(TmpInt);
        Result := True;
      end;
    stDouble:
      begin
        // En caso de que venga como double (por si acaso)
        dt := obj.AsDouble;
        Result := True;
      end;
    stString:
      begin
        TmpStr := obj.AsString;
        if TryISO8601ToDate(TmpStr, dt) then
          Result := True
        else
          Result := TryStrToDateTime(TmpStr, dt); // fallback a formato local
      end;
  else
    Result := False;
  end;
